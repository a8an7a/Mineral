<?php
    ...
    include 'php/resources/checks_end.php';
    include 'php/resources/resize_img.php';
    include 'php/resources/strings_merge.php';
    include 'php/resources/strings_breaks.php';

    /* Запрос на выборку фотографий c тэгом с tags == $tags */
    session_start();
    $tags = $_GET['tags'];

    $query = "SELECT user.UserID, user.Name, user.Fname, user.ProfileImg, img.LinkImg MiniImg, img.LinkImg LinkImg, img.Tags, img.Tags ImgTags FROM user, img WHERE user.UserID = img.UserID AND img.Tags LIKE '%$tags%'";
    ...
    $query  = "SELECT COUNT(*) quantity FROM user, img WHERE user.UserID = img.UserID AND img.Tags LIKE '%$tags%'";
    $result = mysqli_query($link, $query);
...
<body>
    ...
    <div class="search-header">
        <div class="sh-text">
            <?php
                echo 'Найдено '.$count['quantity'].' '.checks_end($count['quantity']).' с тэгом &laquo;'.$tags.'&raquo;';
            ?>
        </div>
    </div>
